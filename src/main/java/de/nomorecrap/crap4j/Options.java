...
public class Options {
  ...
  public void setDownloadAverages(boolean downloadAverages) {
	...
	public String toString() {
		...
		b.append("-p ").append(getProjectDir()).append(" -s ").append(
				join(getSourceDirs())).append(" -c ").append(
				join(getClassDirs())).append(" -t ").append(
				join(getTestClassDirs())).append(" -l ").append(
				join(getLibClasspaths())).append(" -o ").append(getOutputDir())
				.append(" -debug ").append(getDebug()).append(" -dontTest ")
				.append(getDontTest());
		return b.toString();
	}
	...
	private void checkAssignment(int lineNumber, String line) {
		if (!isAssignment(line))
			throw new IllegalArgumentException(
					"Config file is not well-formed. Bad line #(" + lineNumber
							+ "): " + line + ".");
	}
	...
	protected void setProperty(String property, String value) {
		...
		if (property.equals("projectDir"))
			setProjectDir(value);
		else if (property.equals("libClasspaths"))
			setLibClasspaths(value);
		else if (property.equals("testClassDirs"))
			setTestClassDirs(value);
		else if (property.equals("classDirs"))
			setClassDirs(value);
		else if (property.equals("sourceDirs"))
			setSourceDirs(value);
		else
			throw new IllegalArgumentException(
